    ...
    async.parallel([
      ...
      function (callback) {
        fs.readFile(settings.email.jade, 'utf8', callback);
          ...
          // jadetemplate becomes a function to compile the JSON object in argument
          var jadetemplate = jade.compile(data);
          // returns compiled html
          callback(jadetemplate(settings.jade_params));
        });
      ...
      function (err, results) {
        ...
        storedObj.message = message;
        // 2b- ??!!!!
        // 3- SEND TEH FUCKN MAILL11!11
        _send(settings.transport, message, callback);
      }
    );
