...
module.exports = function (path) {
  ...
  jmproto._loadSingleProfile = function (profileName, callback) {
    ...
    var dest_path = self.path+'/'+profileName
      , read_template = ( function (file, callback) {
        ...
        self.profileCache[profileName].paths[extension] = self.profileCache[profileName].paths[extension] || [];
        // WARNING: developing fubar
        // - files object contains one file with an extension type
        // - paths object contains an array of file sharing same extension
        // 
        fs.readFile(dest_path+'/'+file, 'utf8', function (err, data) {
          if (err) { return callback(err); }
          self.profileCache[profileName].files[extension] = data; // change 'file' in 'data'
          self.profileCache[profileName].paths[extension].push(file);
  ...
  jmproto.loadAllProfiles = function (path, callback) {
    ...
    fs.realpath(path, function (err, resolvedPath) {
      if (err) { return callback(new Error('Jademail called with a non-existing path')); }
  ...
  jmproto.Send = function (profileName, emailHeaders, jadeLocals, callback) {
    ...
    var _sendMail = function (profileName, emailHeaders, jadeLocals, callback) {
      var profileUsed = self.profileCache[profileName]
        , compiledTemplate = jade.compile(profileUsed.files['jade'])(jadeLocals)
        , emailMessage = {
        from: emailHeaders.from,
        to: emailHeaders.to,
        subject: emailHeaders.subject,
        text: profileUsed.files['txt'],
        html: juice(compiledTemplate, profileUsed.files['css'])
      };
      ...
      self.transport.sendMail(emailMessage, function(err) {
          ...
          self.emit('emailSent', emailHeaders.to);
          return callback(null);
      });
    };
  ...
  jmproto.reloadProfiles = function (newpath, callback) {
    ...
    if (newpath) {
      self.profileCache = {};
      self.loadAllProfiles(newpath, callback);
    } else {
      self.profileCache = {};
      self.loadAllProfiles(self.path, callback);
    }
  };
  /**
